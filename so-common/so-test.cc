...
template <typename T>
bool test(T& input, T& reference_input, int testsize, gen_type mode)
{
    // the prng needs a nonzero starting state, but if testsize is 0 we don't
    // need to generate anything
    ppc::random rng(testsize, (int) mode);
            ...
            for (int testsize = 0; testsize < 20; ++testsize) {
                ...
                for (int type_i = 0; type_i < (int)gen_types.size(); ++type_i) {
                    ...
                    for (int iter = 0; iter < 10; ++iter) {
                        const auto ok = test(input, reference_input, testsize, mode);
                        if (!ok) {
                            std::printf(CLEAR);
